...
"""
Lumina Video Studio FastAPI Application
...
from api.dependencies import shutdown_lumina_video
...
async def lifespan(app: FastAPI):
    ...
    """
    # Startup
    logger.info("🚀 Starting Lumina Video Studio API...")
    await task_manager.start()
    logger.info("✅ Lumina Video Studio API started successfully\n")
    
    yield
    
    # Shutdown
    logger.info("🛑 Shutting down Lumina Video Studio API...")
    await task_manager.stop()
    await shutdown_lumina_video()
    logger.info("✅ Lumina Video Studio API shutdown complete")


# Create FastAPI app
app = FastAPI(
    title="Lumina Video Studio API",
    description="""
    ## Lumina Video Studio - AI Video Generation Platform API
...
async def root():
    ...
    return {
        "service": "Lumina Video Studio API",
        "version": "0.1.0",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Start Lumina Video Studio API Server")
    ...
    print(f"""
╔══════════════════════════════════════════════════════════════╗
║                    Lumina Video Studio API Server                      ║
╚══════════════════════════════════════════════════════════════╝
