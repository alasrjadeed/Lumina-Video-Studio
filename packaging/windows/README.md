...
Automated build system for creating Windows portable packages of Lumina Video Studio.
...
```
dist/windows/
├── Lumina Video Studio-v*-win64/             # Build directory (version number varies)
│   ├── python/                         # Python embedded
│   ├── tools/                          # FFmpeg, etc.
│   ├── Lumina Video Studio/                  # Project files
...
├── Lumina Video Studio-v*-win64.zip          # ZIP package (version number varies)
└── Lumina Video Studio-v*-win64.zip.sha256   # Checksum (version number varies)
```
...
```bash
# Windows PowerShell
Get-FileHash Lumina Video Studio-v*-win64.zip -Algorithm SHA256
```
...
Same as Lumina Video Studio project license.
