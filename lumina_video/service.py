...
"""
Lumina Video Studio Core - Service Layer
...
from lumina_video.services.llm_service import LLMService
from lumina_video.services.tts_service import TTSService
from lumina_video.services.media import MediaService
from lumina_video.services.api_media import APIProviderMediaService
from lumina_video.services.image_analysis import ImageAnalysisService
from lumina_video.services.video_analysis import VideoAnalysisService
from lumina_video.services.api_asset_analysis import APIAssetAnalysisService
from lumina_video.services.video import VideoService
from lumina_video.services.frame_processor import FrameProcessor
from lumina_video.services.persistence import PersistenceService
from lumina_video.services.history_manager import HistoryManager
from lumina_video.pipelines.standard import StandardPipeline
from lumina_video.pipelines.custom import CustomPipeline
from lumina_video.pipelines.asset_based import AssetBasedPipeline


class LuminaVideoCore:
    """
    Lumina Video Studio Core - Service Layer
    ...
    Usage:
        ...
        media = await lumina_video.media(prompt="a cat")
        
        # Check active capabilities
        print(f"Using LLM: {lumina_video.llm.active}")
        print(f"Available TTS: {lumina_video.tts.available}")
    
    Architecture (Simplified):
        LuminaVideoCore (this class)
    ...
    def __init__(self, config_path: str = "config.yaml"):
        """
        Initialize Lumina Video Studio Core
    ...
    async def _get_or_create_comfykit(self) -> ComfyKit:
        ...
        3. Detects event loop changes (Streamlit reruns) and recreates instance
        4. Ensures proper cleanup of old instances
        ...
        needs_recreate = (
            ...
            or getattr(self, '_comfykit_loop', None) is not current_loop
        )
        
        if needs_recreate:
            # Close old instance if exists
            if self._comfykit is not None:
                reason = "config changed" if self._comfykit_config_hash != current_hash else "event loop changed"
                logger.info(f"🔄 ComfyUI {reason}, recreating ComfyKit instance...")
                try:
    ...
    async def initialize(self):
        ...
        if self._initialized:
            logger.warning("Lumina Video Studio already initialized")
            return
        
        logger.info("🚀 Initializing Lumina Video Studio...")
        ...
        logger.info("✅ Lumina Video Studio initialized successfully\n")
    ...
    def project_name(self) -> str:
        ...
        return self.config.get("project_name", "Lumina Video Studio")
    
    def __repr__(self) -> str:
        """String representation"""
        status = "initialized" if self._initialized else "not initialized"
        pipelines = f"pipelines={list(self.pipelines.keys())}" if self._initialized else ""
        return f"<LuminaVideoCore project={self.project_name!r} status={status} {pipelines}>"


# Global instance
lumina_video = LuminaVideoCore()
